...
if (-not (Test-Path (Join-Path $source "CineVault.exe"))) {
  Write-Host ""
  Write-Host "ERREUR: dossier app\ introuvable a cote de ce script." -ForegroundColor Red
  Write-Host ""
...
Write-Host ""
Write-Host "Installation de CineVault..." -ForegroundColor Cyan
...
$startMenu.Description = "CineVault"
$startMenu.Save()

Write-Host ""
Write-Host "Installation terminee." -ForegroundColor Green
Write-Host "  Dossier : $installDir"
if ($desktopLink) { Write-Host "  Bureau  : $desktopLink" }
Write-Host ""

$launch = Read-Host "Lancer CineVault maintenant ? (O/n)"
