...
const fetchHtml = createCachedHtmlFetcher({
  ...
  getVariants: (url) => [url, url.replace("https://mangaforfree.com", "https://www.mangaforfree.com")],
  buildError: (lastStatus) => `MangaForFree a répondu ${lastStatus || "sans réponse"}`,
});
...
export async function handleMangaforfreeRequest(requestUrl) {
  if (requestUrl.pathname.endsWith("/image")) {
    return fetchProxiedImage(assertMangaforfreeImageUrl(requestUrl.searchParams.get("url") ?? ""), `${BASE_URL}/`, SOURCE_NAME);
  }
