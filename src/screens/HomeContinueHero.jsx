...
export function HomeContinueHero({
  ...
  const EmptyIcon = isVideoContinue ? Clapperboard : BookOpen;

  if (!entry) {
    return (
      <section className="hero-card hero-card--empty" aria-label={t("home.emptyTitle")}>
        <div className="hero-card__glow hero-card__glow--empty" aria-hidden="true" />
        <div className="hero-card__visual hero-card__visual--empty" aria-hidden="true">
          <span className="hero-card__visual-placeholder">
            <EmptyIcon size={28} strokeWidth={1.5} />
          </span>
        </div>
        <div className="hero-card__copy">
          <span className="hero-card__pill">
            <EmptyIcon size={13} aria-hidden="true" />
